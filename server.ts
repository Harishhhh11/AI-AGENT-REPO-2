...
function initPersistence() {
  try {
    ...
    if (fs.existsSync(DB_FILE)) {
      ...
      console.log(`[Storage] Loaded persistent database from ${DB_FILE} (${leads.length} leads, ${appointments.length} appointments, ${conversations.length} conversations)`);
    } else {
      savePersistenceNow();
...
const toolRegistry: Record<string, (args: Record<string, unknown>, orgId: number) => Promise<unknown>> = {
  ...
  search_knowledge: async (args) => {
    ...
    let pool = knowledgeItems.filter((k) => k.is_active);
    if (allowedIds && allowedIds.length > 0) {
      pool = pool.filter((k) => allowedIds.includes(k.id));
    }
...
async function executeReceptionistTurn(
  ...
  const isEnrollmentAnswerTurn =
    ...
    !isExplicitQuestion &&
    !isGibberish &&
    (Boolean(prevAskedEnrollment) ||
      conv?.lead_step === "NAME" ||
      conv?.lead_step === "PHONE" ||
      conv?.lead_step === "MODE" ||
      conv?.lead_step === "BATCH" ||
      conv?.lead_step === "EXPERIENCE") &&
  ...
  const explicitEnrollmentIntent =
    /\b(join|enroll|enrollment|admission|register|apply|sign\s*up|reserve\s+(?:a\s+)?seat|book\s+(?:a\s+)?seat|interested\s+in\s+(?:joining|enrolling)|want\s+to\s+(?:join|enroll|learn))\b/i.test(lower) &&
    !isExplicitQuestion;
  ...
  const shouldCreateOrUpdateLead =
    !isExplicitQuestion &&
    (hasContactInfo || Boolean(isEnrollOrInterestIntent && activeName));
...
app.post("/api/v1/voice/simulate-call", async (req, res) => {
  const { spoken_input, caller_number, agent_id } = req.body;
  const agent = agents.find((a) => a.id === agent_id) || agents[0];
  ...
  const outcome = await executeReceptionistTurn(spoken_input || "Hello, I want information about your AI receptionist.", agent, newConv);

  const newCall: VoiceCallModel = {
    id: getId(),
    organization_id: 1,
    agent_id: agent.id,
    conversation_id: newConv.id,
    call_sid: `CA${Math.random().toString(36).substring(2, 12)}`,
    direction: "inbound",
    caller_number: caller,
    recipient_number: organization.phone,
    status: outcome.isEscalated ? "transferred" : "completed",
    duration_seconds: 45,
    recording_url: null,
    outcome: outcome.isEscalated ? "transferred" : "inquiry_resolved",
    transcript: [
      { role: "caller", text: spoken_input || "Inbound caller greeting", timestamp: "00:04" },
      { role: "assistant", text: outcome.reply, timestamp: "00:08" },
    ],
    created_at: new Date().toISOString(),
  };
...
app.get("/api/v1/knowledge", (req, res) => {
  ...
  if (agentId) {
    items = items.filter((k) => k.agent_id === agentId || k.agent_id === null);
  }
  res.json(items);
});
...
app.post("/api/v1/knowledge/search", async (req, res) => {
  ...
  const searchResults = await toolRegistry.search_knowledge({ query: query || "" }, 1);
